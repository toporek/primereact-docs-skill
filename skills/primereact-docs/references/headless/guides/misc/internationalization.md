...
function ClearFilterButton({ onClear }) {
    ...
    return <button onClick={onClear}>{t('clear')}</button>;
}
```
...
The locale helpers from `primereact/api` were replaced. The [migration guide](../../../primitive/guides/migration/updating-to-v11.md) maps each one to its replacement.
