...
Start from our <a href="https://github.com/primefaces/primereact-examples/tree/main/tanstack-quickstart" target="_blank" rel="noopener noreferrer">TanStack template</a>, pre-configured with PrimeReact.
...
Wrap your app with `PrimeReactProvider` in your root route (`src/routes/__root.tsx`) so PrimeReact features are available across every route. Pass a theme preset such as Aura and your [PrimeUI](https://primeui.dev/pricing) license key to enable styling:

```tsx title="src/routes/__root.tsx" showLineNumbers {2,3,9}
