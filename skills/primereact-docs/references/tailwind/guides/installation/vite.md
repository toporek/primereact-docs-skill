...
Start from our <a href="https://github.com/primefaces/primereact-examples/tree/main/vite-tailwind-quickstart" target="_blank" rel="noopener noreferrer">Vite + Tailwind template</a>, pre-configured with PrimeReact.
...
```bash
npx shadcn@latest init -t vite https://primereact.dev/r/theme.json
```
...
```css showLineNumbers title="src/index.css" {4}
@import 'tailwindcss';
...
Wrap your app with `PrimeReactProvider` in `src/main.tsx` so component behavior and your [PrimeUI](https://primeui.dev/pricing) license are available everywhere. Set `darkModeSelector` to the same `.dark` class used above:

```tsx showLineNumbers title="src/main.tsx" {10,13}
